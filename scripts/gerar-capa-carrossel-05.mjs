// Capa fotografica do carrossel do episodio 4 (parquinho, rotulo T1E04),
// mesmo padrao visual dos episodios anteriores.
import path from 'path';
import sharp from 'sharp';

const SRC = path.resolve('INSTAGRAM OLÁ/Temporada Jornada Olá/Temporada 1/artes-base/05-parquinho-raw.png');
const LOGO_PATH = path.resolve('INSTAGRAM OLÁ/conteudo-inicial/marca-ola-experience-caminho-transparente.png');
const OUT = path.resolve('INSTAGRAM OLÁ/Temporada Jornada Olá/Temporada 1/carrossel-05-parquinho/01-capa.png');

const W = 1080, H = 1080;
const CREAM = '#F3E9D8';
const TERRACOTA = '#C97B4A';

const BG_PAD = '#1A3C30'; // verde-floresta oficial — usado nas faixas de padding

async function main() {
  // 'contain' preserva a largura toda da cena (todas as 6 criancas + os 2 caes),
  // ao inves de 'cover', que cortava simetricamente as laterais e descartava as
  // duas criancas nas pontas (meninos de camiseta azul e coral).
  const cropped = await sharp(SRC)
    .resize(W, H, { fit: 'contain', background: BG_PAD })
    .toBuffer();
  ...
  <svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="70" y="76" class="eyebrow">FAMÍLIA HORIZONTE · T1E04</text>
    <text x="${W - 70}" y="76" text-anchor="end" class="badge">1/4</text>
    <text x="50%" y="${H - 290}" text-anchor="middle" class="headline">
      <tspan x="50%" dy="0">Brincar não tem</tspan>
      <tspan x="50%" dy="58">porta fechada pra ninguém</tspan>
    </text>
