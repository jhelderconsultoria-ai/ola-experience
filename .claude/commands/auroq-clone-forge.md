...
Clona mentes reais em agentes digitais.

Todo conteúdo desta forja vem pelo MCP `arcane` (catálogo `auroq-core`). Chame a tool `forja_bundle` com forge_id `clone-forge` e a fase atual. Nunca procurar arquivo local desta forja; se a tool falhar, mostrar a mensagem e parar — não improvisar de memória.
...
CRITICAL: Do not read `agents/clone-forge/**`. Use the MCP `arcane` tools.
